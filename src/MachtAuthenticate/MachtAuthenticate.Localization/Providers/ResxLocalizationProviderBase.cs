...
namespace MachtAuthenticate.Localization.Providers
{
    ...
    public abstract class ResxLocalizationProviderBase : DependencyObject, ILocalizationProvider
    {
        ...
        protected void TryRemove(string thekey)
        {
            lock (ResourceManagerListLock)
            {
                if (ResourceManagerList.ContainsKey(thekey)) ResourceManagerList.Remove(thekey);
            }
        }
        ...
        protected ResourceManager GetResourceManager(string resourceAssembly, string resourceDictionary)
        {
            ...
            if (AppDomain.CurrentDomain.FriendlyName.Contains("XDesProc") && (now - _lastUpdateCheck).TotalSeconds >= 1.0)
            {
                ...
                var assemblyDir = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly()
                    .Location), "tmp");
                ...
                if (string.IsNullOrEmpty(_projectDirectory))
                {
                    foreach (var process in Process.GetProcesses())
                    {
                        ...
                        if (files.Any(f => Path.GetFileName(f)
                                    .StartsWith(resourceAssembly)))
                        {
                            // We got a hit. Filter the files that are of interest for this provider.
                            _projectFilesCache = files.Where(f => IsFileOfInterest(f, _projectDirectory))
                                .ToArray();
            ...
            if (!TryGetValue(resManKey, out resManager))
            {
                ...
                var possiblePrefixes = availableTypes.Select(tryGetNamespace)
                    .Where(n => n != null)
                    .Distinct()
                    .ToList();
