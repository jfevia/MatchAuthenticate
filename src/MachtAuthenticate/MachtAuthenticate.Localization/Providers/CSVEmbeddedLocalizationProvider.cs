        ...
        public override object GetLocalizedObject(string key, DependencyObject target, CultureInfo culture)
        {
            ...
            foreach (var assemblyInAppDomain in loadedAssemblies)
            {
                ...
                if (assemblyName.Name == assembly)
                {
                    ...
                    filename = assemblyInAppDomain
                        .GetManifestResourceNames()
                        .FirstOrDefault(r => r.Contains($"{dictionary}{(string.IsNullOrEmpty(culture.Name) ? "" : "-")}{culture.Name}"));
                    if (filename != null)
                    {
                        var manifest = assemblyInAppDomain.GetManifestResourceStream(filename);
                        if (manifest == null)
                        {
                            continue;
                        }

                        using (var reader = new StreamReader(manifest, Encoding.Default))
                        {
