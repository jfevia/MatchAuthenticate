    ...
    public class EnumRun : Run
    {
        ...
        private static void PropertiesChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (value != null)
            {
                ...
                if (run.PrependType)
                    key = value.GetType()
                              .Name + run.Separator + key;
                if (!string.IsNullOrEmpty(run.Prefix))
                    key = run.Prefix + run.Separator + key;

                if (run._ext == null)
                {
                    run._ext = new LocalizationResourceExtension();
                    run._ext.Key = key;
                    run._ext.SetBinding(run, run.GetType()
                        .GetProperty("Text"));
                }
                else
                {
        ...
        /// <summary>
        ///     The backing property for <see cref="LocProxy.EnumValue" />
        /// </summary>
        [Category("Common")]
